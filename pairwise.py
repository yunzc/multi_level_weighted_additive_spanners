...
def pairwise2W(G, S):
  ...
  d = math.ceil(math.pow(len(P), 1/3))
  valid_spanner = False
...
def pairwise4W(G, S):
  P = all_pairs_from_subset(S)
  d = math.ceil(math.pow(len(P), 2/7))
  valid_spanner = False
  H = d_light_initialization(G, d)
  path = dict(nx.all_pairs_dijkstra_path(G))
  n = len(list(G.nodes()))
...
def pairwise6W(G, S):
  P = all_pairs_from_subset(S)
  d = math.ceil(math.pow(len(P), 1/4))
  valid_spanner = False
  H = d_light_initialization(G, d)
  path = dict(nx.all_pairs_dijkstra_path(G))
  n = len(list(G.nodes()))
...
def multi_level_spanner(G, subset_arr, stretch):
  total_cost = 0
  l = len(subset_arr)
  E = set()
  for i, T in enumerate(subset_arr):
    if stretch==2:
      Gs, cost = pairwise2W(G, T)
    elif stretch==4:
      Gs, cost = pairwise4W(G, T)
    elif stretch==6:
      Gs, cost = pairwise6W(G, T)
    elif stretch==8:
      Gs, cost = pairwise8W(G, T)
    subset_cost = 0 
    for u, v in Gs.edges():
...
def check_pairwise(folder_name, file_name_without_ext, stretch):
  ...
  subset_cost = multi_level_spanner(G, subset_arr, stretch)
...
if __name__ == '__main__':
  ...
  # 2. File name
  # 3. Output File name
  check_pairwise(sys.argv[1], sys.argv[2], int(sys.argv[3]))
